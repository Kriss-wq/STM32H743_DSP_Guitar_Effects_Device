...
/*
 * Column-oriented GEMV: y[0..out_ch-1] = M * x[0..in_ch-1]
 * M is column-major (out_ch x in_ch): column i starts at M[i * out_ch].
 * Computes y = sum_i( x[i] * M[:, i] ) — each column is contiguous.
 */
...
/* Element-wise vector operations used by bias_add, depthwise, z = a + b */
...
/* Activation functions (fast polynomial approximation) */
static inline float nam_leaky_relu(float x) { return x > 0.0f ? x : 0.01f * x; }

/* ===== SRAM Ring Buffers ===== */
/* All delay-line rings live in AXI SRAM (.ram). Weights / scratch stay in DTCM. */
...
/* Circular ring buffer helpers — power-of-2 sizes, no memmove */

/* Write num_frames of interleaved channel data into the circular buffer.
 * Uses memcpy for the common non-wrapping case; splits into two memcpy
 * calls when the write straddles the wrap boundary. */
                                     ...
                                     int num_frames, int mask)
{
    ...
    int start = wpos & mask;
    int tail = buf_frames - start; /* frames until wrap */
...
void nam_reset(nam_state_t* state) {
    ...
    /* Circular buffers: wpos is staggered per-layer so layers with the
       same buffer size don't all wrap on the same audio block, spreading
       D-cache pressure.  Zero-filled buffer provides zero-padding for
       initial lookback via masked index wrap-around. */
    state->la0_l13_conv_rb_wpos = 682;  /* staggered 1/3 of p2=2048 */
    state->la0_l22_conv_rb_wpos = 1365;  /* staggered 2/3 of p2=2048 */
}

/* Scratch [nf][3] accumulator used by tap-major factored layer functions in la0. */
static float NAM_DTCM la0_tapmajor_tmp[NAM_MAX_BUFFER_SIZE * 3];

/* Weight pointers for la0 layers */
typedef struct {
...
/* Tap-major: each tap's 9 weights load once per call, not once per frame. */
    ...
    int dilation, int nf, int rb_mask)
{
    ...
    *wpos_ptr = (wpos + nf) & rb_mask;
    ...
    /* Tap 0 (d = dilation * 5): initialize tmp from conv_b */
    {
    ...
    /* Tap 1 (d = dilation * 4): accumulate into tmp */
    {
    ...
    /* Tap 2 (d = dilation * 3): accumulate into tmp */
    {
    ...
    /* Tap 3 (d = dilation * 2): accumulate into tmp */
    {
    ...
    /* Tap 4 (d = dilation * 1): accumulate into tmp */
    {
    ...
    /* Tap 5 (d=0) fused with mixin/activation/head_accum/residual */
    {
    ...
    int dilation, int nf, int rb_mask)
{
    ...
    *wpos_ptr = (wpos + nf) & rb_mask;
    ...
    /* Tap 0 (d = dilation * 14): initialize tmp from conv_b */
    {
        const float* const wk = w->conv_w[0];
    ...
    /* Tap 1 (d = dilation * 13): accumulate into tmp */
    {
        const float* const wk = w->conv_w[1];
    ...
    /* Tap 2 (d = dilation * 12): accumulate into tmp */
    {
        const float* const wk = w->conv_w[2];
    ...
    /* Tap 3 (d = dilation * 11): accumulate into tmp */
    {
        const float* const wk = w->conv_w[3];
    ...
    /* Tap 4 (d = dilation * 10): accumulate into tmp */
    {
        const float* const wk = w->conv_w[4];
    ...
    /* Tap 5 (d = dilation * 9): accumulate into tmp */
    {
        const float* const wk = w->conv_w[5];
    ...
    /* Tap 6 (d = dilation * 8): accumulate into tmp */
    {
    ...
    /* Tap 7 (d = dilation * 7): accumulate into tmp */
    {
    ...
    /* Tap 8 (d = dilation * 6): accumulate into tmp */
    {
    ...
    /* Tap 9 (d = dilation * 5): accumulate into tmp */
    {
    ...
    /* Tap 10 (d = dilation * 4): accumulate into tmp */
    {
    ...
    /* Tap 11 (d = dilation * 3): accumulate into tmp */
    {
    ...
    /* Tap 12 (d = dilation * 2): accumulate into tmp */
    {
    ...
    /* Tap 13 (d = dilation * 1): accumulate into tmp */
    {
    ...
    /* Tap 14 (d=0) fused with mixin/activation/head_accum/residual */
    {
...
/* Working buffers in DTCM — keeps D-cache free for SRAM ring buffers */
...
void nam_process(nam_state_t* state, const float* const* input, float* const* output, int num_frames) {
    ...
    /* Copy input to condition buffer (stack / DTCM) */
    for (int f = 0; f < nf; f++)
...
/*
 * Rearrange grouped Conv1x1 weights from C++ load order to column-major.
 * C++ order: for g, for out_i, for in_j -> row-major blocks
 * Column-major: w[in * out_ch + out] = w(out, in)
 */
                                   ...
                                   int in_ch, int out_ch, int groups)
{
    ...
    if (groups == in_ch && in_ch == out_ch) {
        /* Depthwise: just copy per-channel weights */
    ...
    /* Zero the destination (block-diagonal may leave gaps) */
                                  ...
                                  int in_ch, int out_ch, int kernel_size, int groups)
{
    const float* w = *src;
    if (groups == in_ch && in_ch == out_ch) {
        /* Depthwise: C++ order is for c, for k: w[k][c] */
